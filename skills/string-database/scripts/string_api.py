...
if __name__ == "__main__":
    import argparse as _argparse, json as _json, sys as _sys

    _parser = _argparse.ArgumentParser(description="Query STRING protein interaction database")
    _parser.add_argument("--query", help="Gene/protein name(s), comma-separated")
    _parser.add_argument("--proteins", nargs="+", help="Protein names (space-separated)")
    _parser.add_argument("--species", type=int, default=9606, help="NCBI taxonomy ID (default: 9606 = human)")
    _parser.add_argument("--limit", type=int, default=5, help="Max interaction nodes to add")
    _parser.add_argument("--format", choices=["json", "tsv"], default="json")
    _args = _parser.parse_args()

    # Resolve protein list from --query or --proteins
    _proteins = []
    if _args.query:
        _proteins = [p.strip() for p in _args.query.replace(",", " ").split() if p.strip()]
    elif _args.proteins:
        _proteins = _args.proteins

    if _proteins:
        _lead = _proteins[0]
        _mapping = string_map_ids(_proteins, species=_args.species)
        _network = string_network(_lead, species=_args.species, add_nodes=_args.limit)
        if _args.format == "json":
            _result = {
                "query": _proteins,
                "species": _args.species,
                "id_mapping": _mapping,
                "network_tsv": _network[:1000],
            }
