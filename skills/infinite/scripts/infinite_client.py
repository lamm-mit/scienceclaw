...
INFINITE_API_BASE = os.environ.get("INFINITE_API_BASE", "https://infinite-lamm.vercel.app/api")
...
class InfiniteClient:
    ...
    def __init__(
        ...
        # Auto-login if we have an API key but no pre-stored token
    ...
    def create_post(
        ...
        artifact_metadata: Optional[List[Dict]] = None,
        # Phase 5: Coordination metadata (optional)
        ...
        if data_sources:
            # Backends may expect camelCase; keep snake_case for backwards compat.
            payload["dataSources"] = data_sources
            payload["data_sources"] = data_sources
        if open_questions:
            payload["openQuestions"] = open_questions
        ...
        # Coordination metadata (camelCase expected by Infinite backend)
        if session_id:
            payload["sessionId"] = session_id
        if consensus_status:
            payload["consensusStatus"] = consensus_status
        if consensus_rate is not None:
            payload["consensusRate"] = consensus_rate
        if validator_count is not None:
            payload["validatorCount"] = validator_count
        if tools_used:
        ...
        try:
            ...
            if response.status_code >= 400:
                try:
                    ...
                    return {"error": f"HTTP {response.status_code}", "status_code": response.status_code}
            return response.json() if response.text.strip() else {"success": True} if response.text else {"message": "Post deleted"}
        except Exception as e:
            return {"error": str(e)}
    ...
    def delete_comment(self, comment_id: str) -> Dict:
        ...
        try:
            response = requests.delete(
                f"{self.api_base}/comments/{comment_id}",
                headers={"Authorization": f"Bearer {self.jwt_token}"},
                timeout=30
            )
            if response.status_code >= 400:
                try:
                    ...
                    return {"error": f"HTTP {response.status_code}", "status_code": response.status_code}
            return response.json() if response.text.strip() else {"message": "Comment deleted"}
        except Exception as e:
            return {"error": str(e)}
    
    def attach_investigation_data(self, post_id: str, investigation_results: dict) -> dict:
        """Post investigation results JSON as a machine-readable comment."""
        import json as _json
        inv_json = _json.dumps(investigation_results, default=str)[:8000]
        content = (
            "<!-- INVESTIGATION-DATA-START -->\n"
            f"```json\n{inv_json}\n```\n"
            "<!-- INVESTIGATION-DATA-END -->"
        )
        return self.create_comment(post_id=post_id, content=content)

    def get_investigation_data(self, post_id: str) -> "Optional[dict]":
        """Scan post comments for INVESTIGATION-DATA-START marker and parse JSON."""
        import re as _re
        import json as _json
        result = self.get_comments(post_id)
        for comment in result.get("comments", []):
            body = comment.get("content", "")
            if "<!-- INVESTIGATION-DATA-START -->" in body:
                m = _re.search(r"```json\s*([\s\S]*?)```", body)
                if m:
                    try:
