...
CLI Usage:
    python assumption_checks.py --query "residuals" [--input-json '{"data":[...]}'] [--format json]
    python assumption_checks.py --describe-schema
"""
...
INPUT_SCHEMA = {
    ...
    "description": "Normality, outlier, and variance checks on real numeric series.",
    "fallback": "synthetic series from --query",
}


def _load_upstream_data(input_json: str) -> list:
    """Extract numeric data list from --input-json."""
    if not input_json:
        return []
    try:
...
def _make_synthetic_data(query: str) -> list:
    """Generate a synthetic numeric series based on query keyword seed."""
    import hashlib
    seed = int(hashlib.md5(query.encode()).hexdigest(), 16) % (2**32)
    rng = np.random.default_rng(seed)
    return rng.normal(0, 1, 30).tolist()
...
def main():
    ...
    parser.add_argument("--query", "-q", default="data",
                        help="Topic label for synthetic fallback")
    ...
    parser.add_argument("--input-json", default="",
                        help="JSON with upstream data: {data: [number, ...]}")
    ...
    upstream = _load_upstream_data(getattr(args, "input_json", ""))
    if upstream:
        data = np.array(upstream)
        data_source = "upstream"
    else:
        data = np.array(_make_synthetic_data(args.query))
    ...
    if args.format == "json":
        ...
        print("=" * 60)
        print(f"Statistical Assumption Checks — '{args.query[:50]}'")
        print("=" * 60)
        print(f"N            : {result['n']}")
        print(f"Data source  : {data_source}")
        norm = result["normality"]
        print(f"Normality    : {'PASS' if norm['is_normal'] else 'FAIL'}  "
              f"(W={norm['statistic']}, p={norm['p_value']:.4f})")
        out = result["outliers"]
        print(f"Outliers     : {out['n_outliers']} found  "
              f"(bounds [{out['lower_bound']:.3f}, {out['upper_bound']:.3f}])")
        print("=" * 60)
