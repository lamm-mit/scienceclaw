...
# Possible XML filenames inside DrugBank zip (varies by download source)
DRUGBANK_ZIP_XML_NAMES = ("full database.xml", "full_database.xml")
...
def _load_root_from_local(path: str):
    ...
    if p.suffix.lower() == ".zip":
        with zipfile.ZipFile(p) as z:
            namelist = z.namelist()
            xml_name = None
            for candidate in DRUGBANK_ZIP_XML_NAMES:
                if candidate in namelist:
                    xml_name = candidate
                    break
            if xml_name is None:
                # Fallback: first .xml in zip
                xml_name = next((n for n in namelist if n.endswith(".xml")), None)
            if xml_name is None:
                raise ValueError(
                    f"DrugBank zip has no recognized XML (tried {DRUGBANK_ZIP_XML_NAMES}). "
                    f"Names in zip: {namelist[:5]}..."
                )
