...
if __name__ == "__main__":
    ...
    if args.query:
        # Execute the user-supplied query and exit
        entity_type_map = {
            'target': ['target'],
            'disease': ['disease'],
            'drug': ['drug'],
        }
        etype = entity_type_map.get(args.type, ['target'])
        results = search_entities(args.query, entity_types=etype)
        print(f"\nSearch results for '{args.query}' ({args.type}):")
        for r in results[:5]:
            print(f"  {r.get('name', '?')} ({r.get('id', '?')})")
            if args.type == 'target' and r.get('id'):
                try:
                    info = get_target_info(r['id'], include_diseases=True)
                    diseases = info.get('associatedDiseases', {}).get('rows', [])[:3]
                    for d in diseases:
                        print(f"    ↳ {d['disease']['name']} (score: {d.get('score', 0):.2f})")
                except Exception:
                    pass
