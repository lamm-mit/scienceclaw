...
"""CLI client for skill "youtu_agent"."""
import argparse
import json
import sys
...
ARXIV_ID = "2512.24615"
DOCUMENTED_OUTPUT_SHAPE = "{\"response\": \"agent_output\", \"trajectory\": [...]}"


def main() -> None:
    parser = argparse.ArgumentParser(description="Flexible, high-performance framework for building, running, and evaluating autonomous agents with automated generation, experience learning, and RL training capabilities.")
    parser.add_argument("--api-key", type=str, help="API key for authentication", required=True)
    parser.add_argument("--config", type=str, help="Path or name of the agent configuration file (YAML). Examples: 'simple/base_search', 'simple/base', 'ww'", required=True)
    parser.add_argument("--exp-id", type=str, help="Experiment identifier for evaluation runs", default=None)
    parser.add_argument("--dataset", type=str, help="Dataset name for evaluation. Examples: 'WebWalkerQA_15', 'GAIA'", default=None)
    parser.add_argument("--concurrency", type=str, help="Number of concurrent evaluation tasks", default="5")
    parser.add_argument("--config-name", type=str, help="Configuration name for evaluation (e.g., 'ww' for WebWalkerQA)", default=None)
    ...
    if args.config_name is not None:
        params["config_name"] = args.config_name
